...
// 출력박스에 마우스 오버시 함수 실행하기
출력박스[0].onmouseover = () => 김태리[7]("block");
출력박스[0].onmouseout = () => 김태리[7]("none");

// 마우스가 움직일때 김태리 이미지가 따라오게하기
출력박스[0].onmousemove = (이벤트) => {
  // 이벤트 함수에 변수를 하나쓰면 그 변수에 이벤트 객체가 담겨서 들어온다
  console.log(이벤트.offsetX, 이벤트.offsetY);

  //   김태리 이미지의 stale top, left값에 offset값 넣기
...
// 익명함수에 넣어서 값을 보내는 셋팅값을 입력한다
//  ()=> txt[]('');

// 출력박스에 앱솔루트 이미지의 부모 자격주기
...
// 첫번째 .cont에 출력하기
// html컬렉션도 배열과 비슷해서 유사열이라고 함
// 순번을 선택하는 방법이 2가지로 나뉜다.
// 변수.item(순번) or 변수[순번]
출력박스[0].innerHTML += `
이름: ${김태리[5]} /
키: ${김태리[1]} /
...
// 데뷔년도와 같은 배경 안의 배경일 경우
// 특정 배열주소는 대괄호를 연속해서 쓴다
// 배열변수[순번][순번]
//  > 다차원배열이라고 한다. 2개면 2차원 배열
...
const 삼일절 = [
  ...
  function () {
    ...
    // 화살표 함수가 아니면 function() {}일 경우 this 키워드가 4초후 전달되지 않는다.
    // 전달하기 위해 bind() 함수가 필요하다
  }, // 함수 ///
...
// 네번째 출력박스의 값이 2줄이 한 박스에 나오게 40px 로 설정하기
...
const 영화정보보여줘 = function () {
  ...
  // 툴팁 보이기
  출력박스[5].title = `클릭하시면 ${재은.title}예고편을 보실 수 있습니다!`;

  // 예고편 메서드 호출
  출력박스[5].onclick = 재은.trailer;
}; ////// 영화정보보여줘 함수 //////////
